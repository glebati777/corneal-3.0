...
function DigitalTwin({mode, time, selected, onSelect}:{mode:TwinMode;time:number;selected:number;onSelect:(n:number)=>void}){
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      if(mode==="anatomy") drawOCT(ctx,w,h,time,pointer.current.x,pointer.current.y,frame.current);
