...
type TwinMode = "anatomy" | "heat" | "network" | "timeline";
...
function WorldClassTwin({onOpen}:{onOpen:()=>void}){
  const [mode,setMode]=useState<TwinMode>("anatomy"); const [time,setTime]=useState(10); const [selected,setSelected]=useState(0); const [fullscreen,setFullscreen]=useState(false);
  ...
  return <section className={`worldTwin ${fullscreen?"fullscreen":""}`}>
    ...
    <div className="twinBody"><div className="visualStage"><DigitalTwin mode={mode} time={time} selected={selected} onSelect={setSelected}/><div className="stageBadge"><i/><span>Модель обновлена</span><b>14.05.2026</b></div></div>
      <aside className="inspector"><span className="eyebrow">КЛИНИЧЕСКИЙ СИГНАЛ</span><h3>{mode==="network"?signal.name:"Риск отторжения"}</h3><div className="inspectorValue"><strong>{mode==="network"?signal.value:patient.risk}</strong><span>{mode==="network"?signal.unit:"%"}</span></div><p>{mode==="network"?`${signal.group}. Динамика ${signal.delta>0?"+":""}${signal.delta}% относительно предыдущего визита.`:"Неблагоприятная траектория формируется сочетанием иммунных и структурных изменений."}</p><div className="impact"><span>{mode==="network"?"Вклад в прогноз":"Достоверность"}</span><b>{mode==="network"?signal.weight:patient.confidence}%</b><em><i style={{width:`${mode==="network"?signal.weight:patient.confidence}%`}}/></em></div><div className="miniSignals">{top.slice(0,4).map((m,i)=><button key={m.name} onClick={()=>{setMode("network");setSelected(i)}}><span>{m.name}</span><b>{m.weight}%</b></button>)}</div><button className="darkButton" onClick={onOpen}>Открыть исследование<ArrowUpRight/></button></aside>
    </div>
...
function Reports({notify}:{notify:(s:string)=>void}){const [preview,setPreview]=useState<string|null>(null);const reports=[{t:"Полное клиническое заключение",d:"Executive summary, цифровой двойник, полный профиль признаков и план наблюдения.",i:Stethoscope},{t:"Иммунологический профиль",d:"Интерлейкины, хемокины, факторы роста, VEGF и показатели ремоделирования.",i:Dna},{t:"Динамика трансплантата",d:"Траектория риска, визиты, пахиметрия и состояние эндотелия.",i:Activity},{t:"Валидация модели",d:"ROC, калибровка, метрики, важность признаков и ограничения.",i:ShieldCheck}];return <div className="reportGrid">{reports.map(r=>{const Icon=r.i;return <article key={r.t}><div className="reportTop"><div className="reportBadge"><Icon/></div><span>Полная версия</span></div><h2>{r.t}</h2><p>{r.d}</p><div className="reportMeta"><span>40 признаков</span><span>Готов к печати</span></div><div className="reportActions"><button onClick={()=>setPreview(r.t)}><Eye/>Просмотр</button><button onClick={()=>{makeReport(r.t);notify("Отчёт сформирован")}}><Download/>Скачать</button><button onClick={()=>{makeReport(r.t);notify("Открыт файл для печати")}}><Printer/></button></div></article>})}{preview&&<Modal title={preview} close={()=>setPreview(null)}><div className="reportPreview"><span>AURELIA Clinical Intelligence</span><h2>{patient.name}</h2><div className="previewRisk"><small>Риск отторжения</small><strong>{patient.risk}%</strong><b>Достоверность {patient.confidence}%</b></div><h3>Ключевой вывод</h3><p>Рост IL-6, IL-17A, VEGF-A и CXCL10 в сочетании со снижением плотности эндотелия требует контроля через 14 дней.</p><button className="darkButton" onClick={()=>makeReport(preview)}><Download/>Скачать полный отчёт</button></div></Modal>}</div>}
